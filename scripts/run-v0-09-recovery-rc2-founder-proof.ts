...
const disposableDatabasePattern = /^ai_shopping_test_v009_rc2_[a-f0-9]{32}$/;
const successJson = new URL(
  "v0-09-recovery-rc2-founder-proof.json",
  outputDirectory,
);
const successMarkdown = new URL(
  "v0-09-recovery-rc2-founder-proof.md",
  outputDirectory,
);
const failureJson = new URL(
  "v0-09-recovery-rc2-founder-proof-failure.json",
  outputDirectory,
);
const failureMarkdown = new URL(
  "v0-09-recovery-rc2-founder-proof-failure.md",
  outputDirectory,
);
const attemptMarker = new URL(
  "v0-09-recovery-rc2-founder-proof-attempt.json",
  outputDirectory,
);
const releaseModel = "gpt-5.6-terra" as const;
const recoveryCandidate = "RC2" as const;
...
const baseRecoveryHead = "f02560132639e2095356e1ce54afbc87fbded068" as const;
  ...
  for (const candidateListingId of candidateListingIds) {
    ...
    if (run.phase === "first_pass") {
      assertProof(
        ...
        "first-pass run does not use the Recovery RC2 batch policy",
      );
...
async function readCleanRepositoryState(): Promise<CleanRepositoryState> {
  ...
  if (root !== scriptRepositoryRoot) {
    throw new Error(
      "V0-09 Recovery RC2 script is not executing from its bound repository",
    );
  }
  ...
  if (!/^[a-f0-9]{40}$/.test(head)) {
    throw new Error("V0-09 Recovery RC2 could not bind the current Git head");
  }
  if (statusOutput.trim().length > 0) {
    throw new Error(
      "V0-09 Recovery RC2 requires a clean worktree before any release preflight",
    );
  }
  return { root, head };
}
...
async function verifyRepositoryAuthority(state: CleanRepositoryState) {
  if (state.head === baseRecoveryHead) {
    throw new Error(
      "V0-09 Recovery RC2 head is still the preserved RC1 failure checkpoint",
    );
  }
  for (const [authorityName, authorityHead] of [
    ...
    try {
      ...
      throw new Error(
        `V0-09 Recovery RC2 head does not descend from the ${authorityName}`,
      );
    }
  }
  for (const artifact of frozenHistoricalArtifacts) {
    ...
    try {
      ...
      throw new Error(
        `V0-09 Recovery RC2 historical evidence is absent: ${artifact}`,
      );
    }
    if (baseBlob.trim() !== currentBlob.trim()) {
      throw new Error(
        `V0-09 Recovery RC2 historical evidence changed: ${artifact}`,
      );
    }
  }
}
...
async function claimOneShotAttempt(
  ...
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC2 proof has already been attempted or has preserved evidence; refusing to overwrite or consume another release run",
    );
  }
  if (await exists(attemptMarker)) {
    ...
    try {
      ...
      throw new Error("V0-09 Recovery RC2 proof attempt marker is unreadable");
    }
    throw new Error(
      previous.proofMode === "v0_09_recovery_rc2_one_shot" &&
        typeof previous.attemptId === "string"
        ? "V0-09 Recovery RC2 proof has already been claimed; refusing a second run"
        : "V0-09 Recovery RC2 proof marker is not recognised",
    );
  }
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC2 proof has preserved failure evidence without its attempt marker",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC2 repository changed after release preflight began",
    );
  }
  ...
  const marker = {
    schemaVersion: 1,
    proofMode: "v0_09_recovery_rc2_one_shot" as const,
    recoveryCandidate,
    ...
    authorizationReason:
      "Independent review authorized one materially changed Recovery RC2 full founder proof after deterministic full-path hardening.",
    refusalPolicy:
      "This marker is intentionally durable. Recovery RC2 allows exactly one full release proof; a second run is refused. Historical V0-09 attempts remain untouched.",
  };
  try {
    ...
    if (error instanceof Error && "code" in error && error.code === "EEXIST") {
      throw new Error(
        "V0-09 Recovery RC2 proof was claimed concurrently; refusing a second attempt",
      );
    }
    throw error;
  }
...
const databaseName = `ai_shopping_test_v009_rc2_${randomUUID().replaceAll("-", "")}`;
if (
...
if (
  process.env.V0_09_RECOVERY_RC2_LIVE_RELEASE_ACK !==
  "fresh-four-category-one-shot"
) {
  throw new Error(
    "Set V0_09_RECOVERY_RC2_LIVE_RELEASE_ACK=fresh-four-category-one-shot only after every deterministic, database, browser and security gate is green",
  );
}
...
let successReport: Readonly<{
  schemaVersion: 2;
  proofMode: "v0_09_recovery_rc2_one_shot";
  attemptId: string;
  recovery: {
...
if (operationError !== null || cleanupErrors.length > 0) {
  ...
  const failureReport = {
    schemaVersion: 2,
    proofMode: "v0_09_recovery_rc2_one_shot_failed" as const,
    attemptId: attempt.attemptId,
    recovery: {
      candidate: recoveryCandidate,
      acceptedContextHead,
