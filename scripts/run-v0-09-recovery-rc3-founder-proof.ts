...
const disposableDatabasePattern = /^ai_shopping_test_v009_rc3_[a-f0-9]{32}$/;
const successJson = new URL(
  "v0-09-recovery-rc3-founder-proof.json",
  outputDirectory,
);
const successMarkdown = new URL(
  "v0-09-recovery-rc3-founder-proof.md",
  outputDirectory,
);
const failureJson = new URL(
  "v0-09-recovery-rc3-founder-proof-failure.json",
  outputDirectory,
);
const failureMarkdown = new URL(
  "v0-09-recovery-rc3-founder-proof-failure.md",
  outputDirectory,
);
const attemptMarker = new URL(
  "v0-09-recovery-rc3-founder-proof-attempt.json",
  outputDirectory,
);
const releaseModel = "gpt-5.6-terra" as const;
const recoveryCandidate = "RC3" as const;
...
const baseRecoveryHead = "e4ad5b340a21ace15c6bb58dba6d981cc1055d6d" as const;
  ...
  for (const candidateListingId of candidateListingIds) {
    ...
    if (run.phase === "first_pass") {
      assertProof(
        ...
        "first-pass run does not use the Recovery RC3 batch policy",
      );
...
async function readCleanRepositoryState(): Promise<CleanRepositoryState> {
  ...
  if (root !== scriptRepositoryRoot) {
    throw new Error(
      "V0-09 Recovery RC3 script is not executing from its bound repository",
    );
  }
  ...
  if (!/^[a-f0-9]{40}$/.test(head)) {
    throw new Error("V0-09 Recovery RC3 could not bind the current Git head");
  }
  if (statusOutput.trim().length > 0) {
    throw new Error(
      "V0-09 Recovery RC3 requires a clean worktree before any release preflight",
    );
  }
  return { root, head };
}
...
async function verifyRepositoryAuthority(state: CleanRepositoryState) {
  if (state.head === baseRecoveryHead) {
    throw new Error(
      "V0-09 Recovery RC3 head is still the preserved RC2 evidence checkpoint",
    );
  }
  for (const [authorityName, authorityHead] of [
    ...
    try {
      ...
      throw new Error(
        `V0-09 Recovery RC3 head does not descend from the ${authorityName}`,
      );
    }
  }
  for (const artifact of frozenHistoricalArtifacts) {
    ...
    try {
      ...
      throw new Error(
        `V0-09 Recovery RC3 historical evidence is absent: ${artifact}`,
      );
    }
    if (baseBlob.trim() !== currentBlob.trim()) {
      throw new Error(
        `V0-09 Recovery RC3 historical evidence changed: ${artifact}`,
      );
    }
  }
}
...
async function claimOneShotAttempt(
  ...
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC3 proof has already been attempted or has preserved evidence; refusing to overwrite or consume another release run",
    );
  }
  if (await exists(attemptMarker)) {
    ...
    try {
      ...
      throw new Error("V0-09 Recovery RC3 proof attempt marker is unreadable");
    }
    throw new Error(
      previous.proofMode === "v0_09_recovery_rc3_one_shot" &&
        typeof previous.attemptId === "string"
        ? "V0-09 Recovery RC3 proof has already been claimed; refusing a second run"
        : "V0-09 Recovery RC3 proof marker is not recognised",
    );
  }
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC3 proof has preserved failure evidence without its attempt marker",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC3 repository changed after release preflight began",
    );
  }
  ...
  const marker = {
    schemaVersion: 1,
    proofMode: "v0_09_recovery_rc3_one_shot" as const,
    recoveryCandidate,
    ...
    authorizationReason:
      "Independent review authorized one materially changed Recovery RC3 full founder proof after deterministic context semantic correction and full-path hardening.",
    refusalPolicy:
      "This marker is intentionally durable. Recovery RC3 allows exactly one full release proof; a second run is refused. Historical V0-09 attempts and Recovery RC2 evidence remain untouched.",
  };
  try {
    ...
    if (error instanceof Error && "code" in error && error.code === "EEXIST") {
      throw new Error(
        "V0-09 Recovery RC3 proof was claimed concurrently; refusing a second attempt",
      );
    }
    throw error;
  }
...
async function runRc3OracleSelfTest() {
  ...
  const replay = JSON.parse(
    ...
    activeCaseSnapshot?: {
      researchBriefs?: Array<{ brief?: ShoppingBriefV1 }>;
    };
  };
...
if (process.env.V0_09_RC3_ORACLE_SELF_TEST === "1") {
  await runRc3OracleSelfTest();
  process.stdout.write("RC3 oracle self-test passed\n");
  process.exit(0);
}
...
const databaseName = `ai_shopping_test_v009_rc3_${randomUUID().replaceAll("-", "")}`;
if (
...
if (
  process.env.V0_09_RECOVERY_RC3_LIVE_RELEASE_ACK !==
  "fresh-four-category-one-shot"
) {
  throw new Error(
    "Set V0_09_RECOVERY_RC3_LIVE_RELEASE_ACK=fresh-four-category-one-shot only after every deterministic, database, browser and security gate is green",
  );
}
...
let successReport: Readonly<{
  schemaVersion: 2;
  proofMode: "v0_09_recovery_rc3_one_shot";
  attemptId: string;
  recovery: {
...
if (operationError !== null || cleanupErrors.length > 0) {
  ...
  const failureReport = {
    schemaVersion: 2,
    proofMode: "v0_09_recovery_rc3_one_shot_failed" as const,
    attemptId: attempt.attemptId,
    recovery: {
      candidate: recoveryCandidate,
      acceptedContextHead,
...
await Promise.all([
  ...
  writeFile(
    successMarkdown,
    markdown(successReport).replaceAll("Recovery RC2", "Recovery RC3"),
    {
      encoding: "utf8",
      flag: "wx",
    },
