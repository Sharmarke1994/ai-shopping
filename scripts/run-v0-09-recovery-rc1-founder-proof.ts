...
const successJson = new URL(
  "v0-09-recovery-rc1-founder-proof.json",
  outputDirectory,
);
const successMarkdown = new URL(
  "v0-09-recovery-rc1-founder-proof.md",
  outputDirectory,
);
const failureJson = new URL(
  "v0-09-recovery-rc1-founder-proof-failure.json",
  outputDirectory,
);
const failureMarkdown = new URL(
  "v0-09-recovery-rc1-founder-proof-failure.md",
  outputDirectory,
);
const attemptMarker = new URL(
  "v0-09-recovery-rc1-founder-proof-attempt.json",
  outputDirectory,
);
...
async function claimOneShotAttempt() {
  ...
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC1 proof has already been attempted or has preserved evidence; refusing to overwrite or consume another release run",
    );
  }
  if (await exists(attemptMarker)) {
    ...
    try {
      ...
      throw new Error("V0-09 Recovery RC1 proof attempt marker is unreadable");
    }
    throw new Error(
      previous.proofMode === "v0_09_recovery_rc1_one_shot" &&
        typeof previous.attemptId === "string"
        ? "V0-09 Recovery RC1 proof has already been claimed; refusing a second run"
        : "V0-09 Recovery RC1 proof marker is not recognised",
    );
  }
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC1 proof has preserved failure evidence without its attempt marker",
    );
  }
  const marker = {
    schemaVersion: 1,
    proofMode: "v0_09_recovery_rc1_one_shot" as const,
    attemptId: randomUUID(),
    claimedAt: new Date().toISOString(),
    state: "claimed" as const,
    authorizationReason:
      "Independent review authorized one fresh Recovery RC1 full founder proof after the context hardening gate and bounded page-budget correction.",
    refusalPolicy:
      "This marker is intentionally durable. Recovery RC1 allows exactly one full release proof; a second run is refused. Historical V0-09 attempts remain untouched.",
  };
  try {
    ...
    if (error instanceof Error && "code" in error && error.code === "EEXIST") {
      throw new Error(
        "V0-09 Recovery RC1 proof was claimed concurrently; refusing a second attempt",
      );
    }
    throw error;
  }
...
if (
  process.env.V0_09_RECOVERY_RC1_LIVE_RELEASE_ACK !==
  "fresh-four-category-one-shot"
) {
  throw new Error(
    "Set V0_09_RECOVERY_RC1_LIVE_RELEASE_ACK=fresh-four-category-one-shot only after every deterministic, database, browser and security gate is green",
  );
}
...
let successReport: Readonly<{
  schemaVersion: 2;
  proofMode: "v0_09_recovery_rc1_one_shot";
  attemptId: string;
  generatedAt: string;
  models: {
...
if (operationError !== null || cleanupErrors.length > 0) {
  ...
  const failureReport = {
    schemaVersion: 2,
    proofMode: "v0_09_recovery_rc1_one_shot_failed" as const,
    attemptId: attempt.attemptId,
    generatedAt: new Date().toISOString(),
    models: {
      contextAcquisition: releaseModel,
