...
import { extractedProductPageDocumentV1Schema } from "../src/features/product-understanding/page-extraction";
...
} from "../src/features/product-understanding/selection";
import {
...
} from "../src/features/shopping-state/persistence/state-loaders";
...
const disposableDatabasePattern = /^ai_shopping_test_v009_rc4_[a-f0-9]{32}$/;
const successJson = new URL(
  "v0-09-recovery-rc4-founder-proof.json",
  outputDirectory,
);
const successMarkdown = new URL(
  "v0-09-recovery-rc4-founder-proof.md",
  outputDirectory,
);
const failureJson = new URL(
  "v0-09-recovery-rc4-founder-proof-failure.json",
  outputDirectory,
);
const failureMarkdown = new URL(
  "v0-09-recovery-rc4-founder-proof-failure.md",
  outputDirectory,
);
const attemptMarker = new URL(
  "v0-09-recovery-rc4-founder-proof-attempt.json",
  outputDirectory,
);
const releaseModel = "gpt-5.6-terra" as const;
const recoveryCandidate = "RC4" as const;
const acceptedContextHead = "a17d37d80a710bb05b8c79d996596f2492c2424c" as const;
const frozenV009Head = "934067e7d3796a4a68ba3b00387a16632a563f15" as const;
const baseRecoveryHead = "3b67784199769cf2efdf711e5972b784a8d10645" as const;
const rc2PreProofHead = "326b1231031dd292347fa6c3d5af176a4bd85570" as const;
const rc2EvidenceHead = "e4ad5b340a21ace15c6bb58dba6d981cc1055d6d" as const;
const rc2AttemptId = "9d1c6eea-6e66-47f6-95c5-86732b3829fd" as const;
const rc3PreProofHead = "f973041b51ace083f738f56c9e664fcb78e09c95" as const;
const rc3EvidenceHead = "3b67784199769cf2efdf711e5972b784a8d10645" as const;
const rc3AttemptId = "8619d766-d79e-44a6-8a67-39a6c4ac1749" as const;
const frozenHistoricalArtifacts = [
  "docs/evals/v0-09-live-founder-proof-attempt-prior.json",
  "docs/evals/v0-09-live-founder-proof-failure-prior.json",
  "docs/evals/v0-09-live-founder-proof-failure-prior.md",
  "docs/evals/v0-09-live-founder-proof-attempt-2.json",
  "docs/evals/v0-09-live-founder-proof-failure-attempt-2.json",
  "docs/evals/v0-09-live-founder-proof-failure-attempt-2.md",
  "docs/evals/v0-09-live-founder-proof-attempt.json",
  "docs/evals/v0-09-live-founder-proof-failure.json",
  "docs/evals/v0-09-live-founder-proof-failure.md",
  "docs/evals/v0-09-recovery-rc1-founder-proof-attempt.json",
  "docs/evals/v0-09-recovery-rc1-founder-proof-failure.json",
  "docs/evals/v0-09-recovery-rc1-founder-proof-failure.md",
  ...
  "docs/evals/v0-09-recovery-rc3-context-precheck-attempt.json",
  "docs/evals/v0-09-recovery-rc3-context-precheck.json",
  "docs/evals/v0-09-recovery-rc3-context-precheck.md",
  "docs/evals/v0-09-recovery-rc3-founder-proof-attempt.json",
  "docs/evals/v0-09-recovery-rc3-founder-proof-failure.json",
  "docs/evals/v0-09-recovery-rc3-founder-proof-failure.md",
] as const;
...
function createInstrumentedDependencies(options: {
  ...
  const productUnderstandingFailureDiagnostics: Array<
    ProductUnderstandingFailureDiagnostic & { caseName: CaseName }
  > = [];
  ...
  return {
    traces,
    productUnderstandingFailureDiagnostics,
    dependencies: {
      db: options.db,
      model,
      provider,
      ...
      research: {
        ...
        reportProductUnderstandingFailure: (diagnostic) => {
          productUnderstandingFailureDiagnostics.push({
            ...diagnostic,
            caseName: options.activeCase(),
          });
        },
      },
...
function validatedResearchPhase(
  value: string,
): "first_pass" | "deepening" | "reassessment" {
  if (
    value === "first_pass" ||
    value === "deepening" ||
    value === "reassessment"
  ...
  throw new Error("Persisted evidence research phase is invalid");
}
...
async function scopedPersistence(options: {
  ...
  const researchBriefs = await Promise.all(
    [...new Set(taskResearchRuns.map(({ taskRevision }) => taskRevision))]
      .sort((left, right) => (left < right ? -1 : left > right ? 1 : 0))
      .map(async (revision) => ({
        revision,
        brief: projectShoppingBrief(
          await loadShoppingStateAtRevision(options.db, taskId, revision),
        ),
      })),
  );
  ...
  return {
    ...
    researchRuns: taskResearchRuns.map((row) => ({
      ...
      phase: validatedResearchPhase(row.phase),
      status: row.status,
    ...
    fetchedDocuments: documents
      .filter((row) => row.taskId === taskId)
      .map((row) => {
        const document = extractedProductPageDocumentV1Schema.parse(
          row.document,
        );
        return {
          id: row.id,
...
function founderCriterionSemanticallyMatches(options: {
  fixture: CaseFixture;
  phase: "initial" | "refinement";
  key: string;
  item: BriefItem;
...
function assertFounderCriterionSemantics(options: {
  ...
  assertProof(
    founderCriterionSemanticallyMatches(options),
    `${options.fixture.name} ${options.phase} criterion ${options.item.conceptLabel} reversed or invented the founder's ${options.key} semantics`,
  );
}

function classifyFounderCriterion(options: {
  fixture: CaseFixture;
  phase: "initial" | "refinement";
  item: BriefItem;
}) {
  ...
  const rules =
    options.phase === "refinement" && configured.refinement.length > 0
      ...
      : configured.initial;
  const meaning = briefMeaning(options.item);
  const lexicalKeys = rules
    .filter((rule) => rule.meaning.test(meaning))
    .map(({ key }) => key);
  return {
    lexicalKeys,
    semanticKeys: lexicalKeys.filter((key) =>
      founderCriterionSemanticallyMatches({ ...options, key }),
    ),
  };
}
...
function founderIntentOracleProof(options: {
  ...
  const matched = options.brief.items.map((item) => {
    const { lexicalKeys, semanticKeys } = classifyFounderCriterion({
      ...options,
      item,
    });
    if (semanticKeys.length === 0) {
      assertProof(
        lexicalKeys.length > 0,
        `${options.fixture.name} ${options.phase} brief invented an unapproved criterion: ${item.conceptLabel}`,
      );
      // Prefer the first lexically relevant bucket so an invalid strength or
      // value reports the concrete semantic mismatch rather than a generic
      // “invented criterion” message. This is intentionally still strict:
      // assertFounderCriterionSemantics throws below.
      const key = lexicalKeys[0]!;
      assertFounderCriterionSemantics({ ...options, key, item });
      throw new Error("unreachable founder semantic assertion");
    }
    assertProof(
      semanticKeys.length === 1,
      `${options.fixture.name} ${options.phase} brief satisfies multiple founder meanings for ${item.conceptLabel}: ${semanticKeys.join(", ")}`,
    );
    const key = semanticKeys[0]!;
    assertFounderCriterionSemantics({ ...options, key, item });
    return {
  ...
  if (options.fixture.name === "ergonomic-mouse") {
    ...
    const brandQualityFacetCount = brandItems.filter((item) => {
      return (
        item.strength === "hard" &&
    ...
    const amazonBasicsExclusionFacetCount = brandItems.filter((item) => {
      const value = item.semanticValue;
      return (
        item.strength === "hard" &&
        value.kind === "categorical" &&
        ...
        value.values.some(
          (entry) =>
            entry.trim().toLocaleLowerCase("en-GB") === "amazon basics",
        )
      );
    }).length;
    assertProof(
      brandQualityFacetCount === 1 && amazonBasicsExclusionFacetCount === 1,
...
const batchedFirstPassPolicyVersion = `${EVIDENCE_POLICY_VERSION}:${FIRST_PASS_UNDERSTANDING_POLICY_IDENTITY}`;
...
function exactOptionalDate(left: Date | null, right: Date | null) {
  return left?.getTime() === right?.getTime();
}

type ProofAttempt = PersistenceSnapshot["attempts"][number];
type ProofModelReceiptPair = Readonly<{
  candidateListingId: string;
  batchOrdinal: number | null;
  batchTotal: number | null;
  extraction: ProofAttempt;
  assessment: ProofAttempt;
}>;

function modelReceiptPairsForRun(
  persistence: PersistenceSnapshot,
  run: PersistenceSnapshot["researchRuns"][number],
): readonly ProofModelReceiptPair[] {
  ...
  assertProof(
    candidateListingIds.length === run.selectedCandidateCount,
    `${run.phase} run model reservations disagree with selected candidates`,
  );
  const historicalBrief = persistence.researchBriefs.find(
    ({ revision }) => revision === run.taskRevision,
  )?.brief;
  assertProof(
    historicalBrief !== undefined,
    `${run.phase} run lacks its authoritative historical brief`,
  );

  const result: ProofModelReceiptPair[] = [];
  for (const candidateListingId of candidateListingIds) {
    ...
    if (run.phase === "first_pass") {
      assertProof(
        run.policyVersion === batchedFirstPassPolicyVersion,
        "first-pass run does not use the Recovery RC4 batch policy",
      );
      const pairs = pairFirstPassUnderstandingAttempts(candidateAttempts);
      assertFirstPassUnderstandingPairsMatchCriteria(
        pairs,
        historicalBrief.items.map(({ criterionId }) => criterionId),
      );
      assertProof(
        pairs.length <= MAX_FIRST_PASS_MODEL_CALLS_PER_CANDIDATE &&
          pairs.every(
            ({ extraction, assessment }) =>
              extraction.targetCriterionIds.length >= 1 &&
              extraction.targetCriterionIds.length <= 2 &&
              extraction.query === null &&
              assessment.query === null &&
              extraction.provider === "openai" &&
              assessment.provider === "openai" &&
              extraction.model === releaseModel &&
              assessment.model === releaseModel &&
              extraction.promptVersion ===
                PRODUCT_UNDERSTANDING_PROMPT_VERSION &&
              assessment.promptVersion === PRODUCT_UNDERSTANDING_PROMPT_VERSION,
          ),
        "first-pass batches violate their bounded scope or exact provider identity",
      );
      result.push(
        ...pairs.map(({ ordinal, total, extraction, assessment }) => ({
          candidateListingId,
    ...
    assertProof(
      extraction.length === 1 &&
        assessment.length === 1 &&
        exactStringSet(
          extraction[0]!.targetCriterionIds,
          assessment[0]!.targetCriterionIds,
        ) &&
        extraction[0]!.purpose === "combined" &&
        assessment[0]!.purpose === "current_brief" &&
        extraction[0]!.query === null &&
        assessment[0]!.query === null &&
        extraction[0]!.provider === "openai" &&
        assessment[0]!.provider === "openai" &&
        extraction[0]!.model === releaseModel &&
        assessment[0]!.model === releaseModel &&
        extraction[0]!.promptVersion === PRODUCT_UNDERSTANDING_PROMPT_VERSION &&
        assessment[0]!.promptVersion === PRODUCT_UNDERSTANDING_PROMPT_VERSION &&
        extraction[0]!.status === assessment[0]!.status &&
        extraction[0]!.providerRequestId === assessment[0]!.providerRequestId &&
        extraction[0]!.receivedResultCount ===
          assessment[0]!.receivedResultCount &&
        extraction[0]!.failureCode === assessment[0]!.failureCode &&
        exactOptionalDate(extraction[0]!.startedAt, assessment[0]!.startedAt) &&
        exactOptionalDate(extraction[0]!.finishedAt, assessment[0]!.finishedAt),
      `${run.phase} model attempt pair is incoherent`,
    );
    result.push({
      candidateListingId,
      batchOrdinal: null,
  ...
  const runs = persistence.researchRuns.map((run) => {
    ...
    const candidateLimit =
      run.phase === "deepening"
        ? MAX_DEEP_RESEARCH_CANDIDATES
        : MAX_RESEARCH_CANDIDATES;
    const modelPairs = modelReceiptPairsForRun(persistence, run);
    const modelCallsByCandidate = new Map<string, number>();
    for (const pair of modelPairs) {
      modelCallsByCandidate.set(
        pair.candidateListingId,
        (modelCallsByCandidate.get(pair.candidateListingId) ?? 0) + 1,
      );
    }
    assertProof(
    ...
    const expectedSearchCount =
      run.phase === "reassessment" ? 0 : run.selectedCandidateCount;
    assertProof(
      searches.length === run.plannedSearchCount &&
        searches.length === expectedSearchCount &&
    ...
    if (run.phase === "first_pass") {
      assertProof(
        [...modelCallsByCandidate.values()].every(
          (count) => count <= MAX_FIRST_PASS_MODEL_CALLS_PER_CANDIDATE,
        ),
        "first-pass run exceeded its explicit model-call ceiling",
      );
    }
    return {
      ...run,
      ...
      productUnderstandingBatchCount: modelPairs.length,
      maximumProductUnderstandingCallsPerCandidate: Math.max(
        0,
        ...modelCallsByCandidate.values(),
      ),
    };
  });
  return {
    ...
    reassessment: runs.filter(({ phase }) => phase === "reassessment"),
  };
}
...
function terminalPersistenceProof(persistence: PersistenceSnapshot) {
  ...
  assertProof(
    ...
    "an evidence acquisition attempt is absent, non-terminal, or records the wrong understanding provider/model/prompt",
  );
  assertProof(
...
function logicalCallReceiptProof(options: {
  ...
  const modelAttempts = options.persistence.attempts.filter(
    ({ stage }) =>
      stage === "observation_extraction" || stage === "criterion_assessment",
  );
  const modelPairs = options.persistence.researchRuns.flatMap((run) =>
    modelReceiptPairsForRun(options.persistence, run),
  );
  const rows = {
    shopping: {
    ...
    productUnderstanding: {
      ...
      durableBatchReceipts: modelPairs.length,
      durableAttemptRows: modelAttempts.length,
    },
    destination: {
  ...
  assertProof(
    rows.productUnderstanding.logicalCalls ===
      rows.productUnderstanding.durableBatchReceipts &&
      rows.productUnderstanding.durableAttemptRows === modelPairs.length * 2,
    "model call count disagrees with durable extraction/assessment batch receipts",
  );
  assertProof(
...
type CaseUnderstandingDiagnostic = ProductUnderstandingFailureDiagnostic & {
  caseName: CaseName;
};

function productUnderstandingFailureSignature(options: {
  candidateListingId: string;
...
function productUnderstandingFailureProof(options: {
  fixture: CaseFixture;
  persistence: PersistenceSnapshot;
  diagnostics: readonly CaseUnderstandingDiagnostic[];
}) {
  const expected = options.persistence.researchRuns.flatMap((run) =>
    modelReceiptPairsForRun(options.persistence, run)
      .filter(({ extraction }) => extraction.status === "failed")
      .map(({ candidateListingId, extraction }) =>
        productUnderstandingFailureSignature({
          candidateListingId,
          researchPhase: run.phase,
          requireCriterionBinding:
            run.phase === "first_pass" || run.phase === "deepening",
          criterionCount: extraction.targetCriterionIds.length,
          failureCode: extraction.failureCode,
          providerRequestId: extraction.providerRequestId,
        }),
      ),
  );
  const actual = options.diagnostics.map((diagnostic) => {
    assertProof(
      diagnostic.caseName === options.fixture.name,
      "product-understanding diagnostic was attributed to the wrong category",
    );
    return productUnderstandingFailureSignature({
      candidateListingId: diagnostic.candidateListingId,
      researchPhase: diagnostic.researchPhase,
      requireCriterionBinding: diagnostic.requireCriterionBinding,
      criterionCount: diagnostic.criterionCount,
  ...
  assertProof(
    ...
    "failed model receipts disagree with sanitized failure diagnostics",
  );
  return {
    failedBatchReceiptCount: expected.length,
    diagnosticCount: actual.length,
...
async function runCase(options: {
  ...
  productUnderstandingFailureDiagnostics: CaseUnderstandingDiagnostic[];
  ...
  const diagnosticStart = options.productUnderstandingFailureDiagnostics.length;
  ...
  const productUnderstandingFailures = productUnderstandingFailureProof({
    fixture: options.fixture,
    persistence,
    diagnostics:
      options.productUnderstandingFailureDiagnostics.slice(diagnosticStart),
  });
  const concurrency = {
...
function markdown(report: {
  ...
  productUnderstandingFailureDiagnostics: readonly (ProductUnderstandingFailureDiagnostic & {
    caseName: CaseName;
  })[];
}) {
  const diagnosticRules = Object.entries(
    report.productUnderstandingFailureDiagnostics.reduce<
...
type CleanRepositoryState = Readonly<{ root: string; head: string }>;

async function readCleanRepositoryState(): Promise<CleanRepositoryState> {
  const { stdout: rootOutput } = await executeFile(
    "git",
    ["rev-parse", "--show-toplevel"],
    { cwd: scriptRepositoryRoot },
  );
  const root = resolve(rootOutput.trim());
  if (root !== scriptRepositoryRoot) {
    throw new Error(
      "V0-09 Recovery RC4 script is not executing from its bound repository",
    );
  }
  const [{ stdout: headOutput }, { stdout: statusOutput }] = await Promise.all([
    executeFile("git", ["rev-parse", "HEAD"], { cwd: root }),
  ...
  if (!/^[a-f0-9]{40}$/.test(head)) {
    throw new Error("V0-09 Recovery RC4 could not bind the current Git head");
  }
  if (statusOutput.trim().length > 0) {
    throw new Error(
      "V0-09 Recovery RC4 requires a clean worktree before any release preflight",
    );
  }
  return { root, head };
}

async function verifyRepositoryAuthority(state: CleanRepositoryState) {
  if (state.head === baseRecoveryHead) {
    throw new Error(
      "V0-09 Recovery RC4 head is still the preserved RC3 evidence checkpoint",
    );
  }
  for (const [authorityName, authorityHead] of [
    ["preserved RC1 checkpoint", baseRecoveryHead],
    ["accepted context checkpoint", acceptedContextHead],
    ["frozen V0-09 checkpoint", frozenV009Head],
  ] as const) {
    try {
      await executeFile(
        "git",
        ["merge-base", "--is-ancestor", authorityHead, state.head],
        { cwd: state.root },
      );
    } catch {
      throw new Error(
        `V0-09 Recovery RC4 head does not descend from the ${authorityName}`,
      );
    }
  }
  for (const artifact of frozenHistoricalArtifacts) {
    let baseBlob: string;
    let currentBlob: string;
    try {
      [{ stdout: baseBlob }, { stdout: currentBlob }] = await Promise.all([
        executeFile("git", ["rev-parse", `${baseRecoveryHead}:${artifact}`], {
          cwd: state.root,
        }),
        executeFile("git", ["rev-parse", `${state.head}:${artifact}`], {
          cwd: state.root,
        }),
      ]);
    } catch {
      throw new Error(
        `V0-09 Recovery RC4 historical evidence is absent: ${artifact}`,
      );
    }
    if (baseBlob.trim() !== currentBlob.trim()) {
      throw new Error(
        `V0-09 Recovery RC4 historical evidence changed: ${artifact}`,
      );
    }
  }
}
...
async function claimOneShotAttempt(
  preflightRepositoryState: CleanRepositoryState,
) {
  ...
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC4 proof has already been attempted or has preserved evidence; refusing to overwrite or consume another release run",
    );
  }
  if (await exists(attemptMarker)) {
    let previous: { proofMode?: unknown; attemptId?: unknown };
    try {
      ...
      throw new Error("V0-09 Recovery RC4 proof attempt marker is unreadable");
    }
    throw new Error(
      previous.proofMode === "v0_09_recovery_rc4_one_shot" &&
        typeof previous.attemptId === "string"
        ? "V0-09 Recovery RC4 proof has already been claimed; refusing a second run"
        : "V0-09 Recovery RC4 proof marker is not recognised",
    );
  }
  if (
    ...
    throw new Error(
      "V0-09 Recovery RC4 proof has preserved failure evidence without its attempt marker",
    );
  }
  const repositoryState = await readCleanRepositoryState();
  if (
    repositoryState.root !== preflightRepositoryState.root ||
    repositoryState.head !== preflightRepositoryState.head
  ) {
    throw new Error(
      "V0-09 Recovery RC4 repository changed after release preflight began",
    );
  }
  await verifyRepositoryAuthority(repositoryState);
  const recoveryHead = repositoryState.head;
  const marker = {
    schemaVersion: 1,
    proofMode: "v0_09_recovery_rc4_one_shot" as const,
    recoveryCandidate,
    ...
    recoveryHead,
    reason:
      "RC1 exposed persisted page-byte validation using the obsolete 1.5 MB bound after transport policy moved to 2.5 MB.",
    authorizationReason:
      "Independent review authorized one materially changed Recovery RC4 full founder proof after deterministic evaluator hardening and full-path hardening.",
    refusalPolicy:
      "This marker is intentionally durable. Recovery RC4 allows exactly one full release proof; a second run is refused. Historical V0-09 attempts and Recovery RC3 evidence remain untouched.",
  };
  try {
    ...
    if (error instanceof Error && "code" in error && error.code === "EEXIST") {
      throw new Error(
        "V0-09 Recovery RC4 proof was claimed concurrently; refusing a second attempt",
      );
    }
    throw error;
  }
...
async function runRc4OracleSelfTest() {
  const fixture = cases[0]!;
  const item = (options: {
    label: string;
    definition: string;
    text: string;
    strength: BriefItem["strength"];
    value?: unknown;
    targetSemantics?: BriefItem["targetSemantics"];
  }) =>
    ({
      criterionId: "00000000-0000-4000-8000-000000000001",
      lineageId: "00000000-0000-4000-8000-000000000002",
      conceptId: "00000000-0000-4000-8000-000000000003",
      conceptLabel: options.label,
      conceptDefinition: options.definition,
      strength: options.strength,
      targetSemantics: options.targetSemantics ?? "qualitative",
      semanticValue: options.value ?? {
        schemaVersion: 1,
        kind: "qualitative",
        mode: "text",
        text: options.text,
  ...
  const brief = (
    ...
    return {
      ...
      items,
    } as unknown as ShoppingBriefV1;
  };
  const assertFails = (run: () => void, message: string) => {
    let failed = false;
    try {
      run();
    } catch {
    ...
    assertProof(failed, message);
  };
  const classify = (candidate: BriefItem) =>
    classifyFounderCriterion({ fixture, phase: "initial", item: candidate });
  const ergonomicPreference = item({
    label: "Ergonomic design",
    definition: "Whether the mouse is designed with an ergonomic shape.",
    text: "ergonomic",
    strength: "preference",
  });
  const ergonomicClassification = classify(ergonomicPreference);
  assertProof(
    JSON.stringify(ergonomicClassification.semanticKeys) ===
      JSON.stringify(["ergonomic_subject"]),
    "RC3 oracle self-test: ergonomic preference did not disambiguate to ergonomic_subject",
  );
  const sculptedProfile = item({
    label: "Mouse shape and profile",
    definition:
      "The mouse is chunkier and sculpted with a side profile and thumb-rest.",
    text: "chunkier and sculpted, with a noticeable side profile or thumb-rest shape",
    strength: "preference",
  });
  const sculptedClassification = classify(sculptedProfile);
  assertProof(
    JSON.stringify(sculptedClassification.semanticKeys) ===
      JSON.stringify(["sculpted_shape"]),
    "RC3 oracle self-test: sculpted profile did not classify independently",
  );
  const hardErgonomicClassification = classify(
    item({
      label: "Ergonomic design",
      definition: "Whether the mouse is designed with an ergonomic shape.",
      text: "ergonomic",
  ...
  assertProof(
    hardErgonomicClassification.lexicalKeys.includes("ergonomic_subject") &&
      hardErgonomicClassification.semanticKeys.length === 0,
    "RC3 oracle self-test: hard ergonomic criterion was not isolated as a semantic-strength failure",
  );
  let strengthFailure = "";
  try {
    assertFounderCriterionSemantics({
      fixture,
      phase: "initial",
      key: "ergonomic_subject",
      item: item({
        label: "Ergonomic design",
        definition: "Whether the mouse is designed with an ergonomic shape.",
        text: "ergonomic",
    ...
    strengthFailure = error instanceof Error ? error.message : String(error);
  }
  assertProof(
    /ergonomic_subject semantics/.test(strengthFailure) &&
      !/invented an unapproved criterion/.test(strengthFailure),
    "RC3 oracle self-test: hard ergonomic failure was reported as invention",
  );
  const bothClassification = [ergonomicPreference, sculptedProfile].map(
    classify,
  );
  assertProof(
    JSON.stringify(
      bothClassification.map(({ semanticKeys }) => semanticKeys),
    ) === JSON.stringify([["ergonomic_subject"], ["sculpted_shape"]]),
    "RC3 oracle self-test: independently valid ergonomic and sculpted criteria collided",
  );
  const unrelatedClassification = classify(
    item({
      label: "Favourite colour",
      definition: "A colour the shopper likes.",
      text: "blue",
      strength: "preference",
    }),
  );
  assertProof(
    unrelatedClassification.lexicalKeys.length === 0,
    "RC3 oracle self-test: unrelated criterion was accepted",
  );

  const validOfficeChair = brief("office-chair", [
    item({
      label: "Price",
      definition: "The chair purchase budget.",
      text: "",
      strength: "preference",
    ...
    item({
      ...
      definition: "Comfort during long sessions working from home.",
      text: "comfortable for long sessions",
      strength: "preference",
    }),
    item({
      label: "Lower-back support",
      definition: "Good lower-back support.",
      text: "good lower-back support",
      strength: "strong_preference",
    }),
    item({
      label: "Breathable material",
      definition: "Mesh or fabric breathable material.",
      text: "",
      strength: "preference",
      value: {
        schemaVersion: 1,
    ...
    item({
      label: "Physical size",
      definition: "The chair size and footprint; not huge.",
      text: "",
      strength: "preference",
      value: {
        schemaVersion: 1,
    ...
    item({
      label: "Non-gamer style",
      definition: "A not gamer-looking chair style.",
      text: "",
      strength: "preference",
      value: {
        schemaVersion: 1,
    ...
    item({
      label: "Shopper fit",
      definition: "Shopper height is 5'10.",
      text: "",
      strength: "preference",
  ...
  founderIntentOracleProof({
    fixture: cases.find(({ name }) => name === "office-chair")!,
    brief: validOfficeChair,
    phase: "initial",
  });

  const validVacuum = brief("cordless-vacuum", [
    item({
      label: "Cordless operation",
      definition: "The vacuum's cordless operation.",
      text: "",
      strength: "preference",
    ...
    item({
      label: "Price",
      definition: "The vacuum purchase budget.",
      text: "",
      strength: "hard",
      targetSemantics: "range",
      value: {
    ...
    item({
      ...
      definition: "Hard floor and rug suitability.",
      text: "",
      strength: "hard",
      ...
      value: {
        ...
        kind: "categorical",
        operator: "include",
        values: ["hard floor", "rug"],
      },
    }),
    item({
      label: "Noise",
      definition: "The vacuum is quiet with low noise.",
      text: "quiet low noise",
      strength: "hard",
    }),
    item({
      label: "Weight",
      definition: "Vacuum weight under 3kg.",
      text: "",
      strength: "preference",
      targetSemantics: "range",
      value: {
        ...
        lower: undefined,
        upper: { amount: "3000", inclusive: false },
        unit: "g",
      },
    }),
    item({
      label: "Runtime",
      definition: "At least 40 minutes useful runtime.",
      text: "at least 40 minutes useful runtime",
      strength: "preference",
    }),
    item({
      label: "Small flat context",
      definition: "Storage in a small flat.",
      text: "small flat storage context",
      strength: "preference",
    }),
  ]);
  founderIntentOracleProof({
    fixture: cases.find(({ name }) => name === "cordless-vacuum")!,
    brief: validVacuum,
    phase: "initial",
  });

  const validCoffee = brief("compact-coffee-machine", [
    item({
      label: "Price",
      definition: "The coffee machine purchase budget.",
      text: "",
      strength: "hard",
      targetSemantics: "range",
      value: {
    ...
    item({
      label: "Compact kitchen fit",
      definition: "Compact machine for a small kitchen.",
      text: "compact small kitchen",
      strength: "preference",
    }),
    item({
      label: "Maximum width",
      ...
      value: {
        ...
        kind: "measurement_range",
        lower: undefined,
        upper: { amount: "250", inclusive: true },
        unit: "mm",
      },
    }),
    item({
      label: "Espresso quality",
      definition: "Good espresso quality.",
      text: "good espresso",
      strength: "strong_preference",
    }),
    item({
      label: "Cleaning",
      definition: "Easy cleaning and maintenance.",
      text: "easy to clean",
      strength: "preference",
    }),
    item({
      label: "Noise",
      definition: "A quieter machine with low noise.",
      text: "quieter low noise",
      strength: "preference",
    }),
    item({
      label: "Milk frothing",
      definition: "Milk frothing is useful but not essential.",
      text: "milk frothing useful but not essential",
      strength: "preference",
    }),
  ]);
  founderIntentOracleProof({
    fixture: cases.find(({ name }) => name === "compact-coffee-machine")!,
    brief: validCoffee,
    phase: "initial",
  });
  assertFails(
    () =>
      founderIntentOracleProof({
        ...
        brief: {
          ...validOfficeChair,
          items: validOfficeChair.items.map((candidate) =>
            candidate.conceptLabel === "Lower-back support"
              ? { ...candidate, strength: "preference" as const }
              : candidate,
          ),
        },
        phase: "initial",
      }),
    "RC4 oracle self-test: softened lower-back support was accepted",
  );
  assertFails(
    () =>
      founderIntentOracleProof({
        ...
        brief: {
          ...validVacuum,
          items: validVacuum.items.map((candidate) =>
            candidate.conceptLabel === "Floor coverage"
              ? {
                  ...candidate,
                  semanticValue: {
                    schemaVersion: 1,
    ...
    "RC4 oracle self-test: incomplete floor coverage was accepted",
  );
  assertFails(
    () =>
      founderIntentOracleProof({
        ...
        brief: {
          ...validCoffee,
          items: validCoffee.items.map((candidate) =>
            candidate.conceptLabel === "Maximum width"
              ? {
                  ...candidate,
                  semanticValue: {
                    schemaVersion: 1,
                    kind: "measurement_range",
                    lower: undefined,
                    upper: { amount: "300", inclusive: true },
                    unit: "mm",
                  } as const,
                }
              : candidate,
    ...
    "RC4 oracle self-test: widened coffee-machine width was accepted",
  );
  ...
  const replay = JSON.parse(
    ...
    activeCaseSnapshot?: {
      researchBriefs?: Array<{ revision?: string; brief?: ShoppingBriefV1 }>;
    };
  };
  const replayBrief = replay.activeCaseSnapshot?.researchBriefs?.[0]?.brief;
  assertProof(
    replayBrief !== undefined,
    "RC3 oracle self-test: sanitized RC2 replay brief is unavailable",
  );
  let replayFailure = "";
  try {
    founderIntentOracleProof({ fixture, brief: replayBrief, phase: "initial" });
  } catch (error) {
    replayFailure = error instanceof Error ? error.message : String(error);
  }
  assertProof(
    /ergonomic_subject semantics/.test(replayFailure) &&
      !/invented an unapproved criterion/.test(replayFailure),
    "RC3 oracle self-test: sanitized RC2 replay did not fail at ergonomic strength",
  );
  const correctedReplayBrief = {
    ...replayBrief,
    items: [
      ...replayBrief.items.map((candidate) =>
        candidate.conceptLabel === "Ergonomic design"
          ? { ...candidate, strength: "preference" as const }
          : candidate.conceptLabel === "Brand reputation"
            ? {
                ...candidate,
                semanticValue: {
                  schemaVersion: 1,
      ...
      item({
        ...
        definition: "How strong the mouse reviews and ratings are.",
  ...
  founderIntentOracleProof({
    fixture,
    brief: correctedReplayBrief,
    phase: "initial",
  });
  const brandQuality = correctedReplayBrief.items.find(
    ({ conceptLabel }) => conceptLabel === "Brand reputation",
  );
  const amazonBasics = correctedReplayBrief.items.find(
    ({ conceptLabel }) => conceptLabel === "Brand",
  );
  assertProof(
    brandQuality !== undefined && amazonBasics !== undefined,
    "RC4 oracle self-test: corrected replay lacks the two brand facets",
  );
  assertFails(
    () =>
      founderIntentOracleProof({
        fixture,
        brief: {
          ...correctedReplayBrief,
          items: correctedReplayBrief.items.filter(
            ({ conceptLabel }) => conceptLabel !== "Brand",
          ),
        },
        phase: "initial",
      }),
    "RC4 oracle self-test: missing Amazon Basics exclusion was accepted",
  );
  assertFails(
    () =>
      founderIntentOracleProof({
    ...
    "RC4 oracle self-test: missing brand quality was accepted",
  );
  assertFails(
    () =>
      founderIntentOracleProof({
        ...
        brief: {
          ...correctedReplayBrief,
          items: correctedReplayBrief.items.map((candidate) =>
            candidate === amazonBasics
              ? {
                  ...candidate,
                  semanticValue: {
                    schemaVersion: 1,
    ...
    "RC4 oracle self-test: unsupported concrete brand exclusion was accepted",
  );
  assertFails(
    () =>
      founderIntentOracleProof({
    ...
    "RC4 oracle self-test: softened brand quality was accepted",
  );
  assertFails(
    () =>
      founderIntentOracleProof({
        ...
        brief: {
          ...correctedReplayBrief,
          items: [...correctedReplayBrief.items, amazonBasics],
        },
        phase: "initial",
      }),
    "RC4 oracle self-test: duplicate Amazon Basics exclusion was accepted",
  );

  const rc3Replay = JSON.parse(
    await readFile(
      new URL(
        "../docs/evals/v0-09-recovery-rc3-founder-proof-failure.json",
        import.meta.url,
  ...
  const rc3InitialBrief = rc3Replay.activeCaseSnapshot?.researchBriefs?.find(
    ({ revision }) => revision === "1",
  )?.brief;
  const rc3RefinementBrief = rc3Replay.activeCaseSnapshot?.researchBriefs?.find(
    ({ revision }) => revision === "2",
  )?.brief;
  assertProof(
    rc3InitialBrief !== undefined && rc3RefinementBrief !== undefined,
    "RC4 oracle self-test: RC3 preserved briefs are unavailable",
  );
  founderIntentOracleProof({
    fixture,
    brief: rc3InitialBrief,
    phase: "initial",
  });
  founderIntentOracleProof({
    fixture,
    brief: rc3RefinementBrief,
    phase: "refinement",
  });
}

if (process.env.V0_09_RC4_ORACLE_SELF_TEST === "1") {
  await runRc4OracleSelfTest();
  process.stdout.write("RC4 oracle self-test passed\n");
  process.exit(0);
}

const preflightRepositoryState = await readCleanRepositoryState();
await verifyRepositoryAuthority(preflightRepositoryState);
...
const databaseName = `ai_shopping_test_v009_rc4_${randomUUID().replaceAll("-", "")}`;
if (
...
if (
  process.env.V0_09_RECOVERY_RC4_LIVE_RELEASE_ACK !==
  "fresh-four-category-one-shot"
) {
  throw new Error(
    "Set V0_09_RECOVERY_RC4_LIVE_RELEASE_ACK=fresh-four-category-one-shot only after every deterministic, database, browser and security gate is green",
  );
}
...
let successReport: Readonly<{
  schemaVersion: 2;
  proofMode: "v0_09_recovery_rc4_one_shot";
  attemptId: string;
  recovery: {
    candidate: typeof recoveryCandidate;
    acceptedContextHead: typeof acceptedContextHead;
    frozenV009Head: typeof frozenV009Head;
    rc2PreProofHead: typeof rc2PreProofHead;
    rc2EvidenceHead: typeof rc2EvidenceHead;
    rc2AttemptId: typeof rc2AttemptId;
    rc3PreProofHead: typeof rc3PreProofHead;
    rc3EvidenceHead: typeof rc3EvidenceHead;
    rc3AttemptId: typeof rc3AttemptId;
    baseRecoveryHead: typeof baseRecoveryHead;
    frozenHistoricalArtifacts: typeof frozenHistoricalArtifacts;
    recoveryHead: string;
  };
  generatedAt: string;
  models: {
...
try {
  attempt = await claimOneShotAttempt(preflightRepositoryState);
} catch (error) {
  const cleanup = await cleanupProofResources({
    connection,
    admin,
...
try {
  for (const fixture of cases) {
    ...
    completedFlows.push(
      await runCase({
        ...
        productUnderstandingFailureDiagnostics:
          releaseRuntime.productUnderstandingFailureDiagnostics,
        onSession: (sessionId) => {
  ...
  successReport = {
    ...
    recovery: {
      ...
      baseRecoveryHead,
      frozenHistoricalArtifacts,
      recoveryHead: attempt.recoveryHead,
    },
...
if (operationError !== null || cleanupErrors.length > 0) {
  ...
  const failureReport = {
    schemaVersion: 2,
    proofMode: "v0_09_recovery_rc4_one_shot_failed" as const,
    attemptId: attempt.attemptId,
    recovery: {
      candidate: recoveryCandidate,
      acceptedContextHead,
  ...
  await Promise.all([
    ...
    writeFile(
      failureMarkdown,
      `# V0-09 Recovery RC3 founder proof — failed\n\nGenerated: ${failureReport.generatedAt}\n\n- Failed case: ${failureReport.failedCase ?? "before a category started"}\n- Completed categories: ${failureReport.completedFlows.map(({ name }) => name).join(", ") || "none"}\n- Active-case state captured: ${failureReport.activeCaseSnapshot === null ? "no" : "yes"}\n- Interpretation diagnostics captured: ${failureReport.contextInterpretationDiagnostics.length}\n- Product-understanding diagnostics captured: ${failureReport.productUnderstandingFailureDiagnostics.length}\n- Product-understanding failure rules: ${[...new Set(failureReport.productUnderstandingFailureDiagnostics.map(({ rule }) => rule))].sort().join(", ") || "none"}\n- Disposable database destroyed: ${failureReport.cleanup.disposableDatabaseDestroyed ? "yes" : "no"}\n- Sanitized failure: ${failureReport.failure.name}: ${failureReport.failure.message}\n- Release accepted: no\n\nThis single Recovery RC3 attempt is preserved as diagnostic evidence only. Historical V0-09 attempts and Recovery RC2 evidence remain untouched and a second RC3 proof is refused.\n`,
      { encoding: "utf8", flag: "wx" },
    ),
  ]);
  const failureMarkdownText = await readFile(failureMarkdown, "utf8");
  await writeFile(
    failureMarkdown,
    failureMarkdownText
      .replaceAll("Recovery RC2", "Recovery RC4")
      .replaceAll("Recovery RC3", "Recovery RC4"),
    { encoding: "utf8" },
  );
