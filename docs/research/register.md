...
| OpenAI Structured Outputs can enforce a supplied JSON schema, while refusals and incomplete Responses remain explicit non-success branches. | V0-05 can use strict structured output but must still handle refusal, incomplete status, and local validation before applying anything. [OpenAI Structured Outputs documentation](https://developers.openai.com/api/docs/guides/structured-outputs) |
